...
def generate_math_funs(filename: str) -> None:
    with open(filename, 'a') as f:
        f.write("float exp_approx(float x, int n_term) { int i; float approx, power, fact; approx = power = fact = 1; for (i = 0; i < n_term; i++) { power *= x; fact *= i+1; approx += power / fact; } return approx; }\n")
        f.write("float sigmoid(float x) { return (1 / (1 + exp_approx(-x, 10))); }\n\n")
...
if __name__ == "__main__":
    ...
    filename = "logistic_model_predict.c"
    with open(filename, 'w') as f:
        f.write("#include <stdio.h>\n")
        f.write("#include <stdlib.h>\n\n")
    generate_math_funs(filename)
